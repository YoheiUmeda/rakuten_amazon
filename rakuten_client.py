...
def escape_rakuten_keyword(
    ...
    if not keyword or not keyword.strip():
        print("[楽天 keyword] 空入力 → fallback を使用")
        return fallback
    ...
    if not tokens:
        print("[楽天 keyword] トークン除去後に何も残らない → fallback")
        return fallback
    scored_tokens = []
    for token in tokens:
        score = 0
    ...
    if not keyword or len(encoded.encode('utf-8')) > byte_limit:
        print(f"[楽天 keyword] エンコード後にバイト長オーバー → fallback: {fallback}")
        return fallback
    # if len(keyword) < 4 or re.fullmatch(r'[a-zA-Z0-9\- ]+', keyword):
    #     print(f"[楽天 keyword] 内容が希薄/単純文字列 → fallback: {fallback}")
        # return fallback
    if len(keyword) < 4:
        return fallback  # 長さチェックのみ残す
    print(f"[楽天キーワード生成] 原文: {original} → 使用: {keyword}")
    return keyword
...
def perform_rakuten_api_search_from_itemcode(itemcode: str, app_id: str):
    ...
    try:
        r = requests.get(url, params=params)
        if r.status_code == 429 or ('error' in r.text and 'too_many_requests' in r.text):
            print("[楽天API] レート制限, スリープしてリトライ")
            time.sleep(10)
            return perform_rakuten_api_search_from_itemcode(itemcode, app_id)
        if r.status_code != 200:
            print(f"[楽天APIエラー] status={r.status_code} itemcode={itemcode}")
            return []
        js = r.json()
        if "error" in js:
            print(f"[楽天APIエラー] error={js['error']} itemcode={itemcode}")
            if js["error"] == "too_many_requests":
                time.sleep(10)
                return perform_rakuten_api_search_from_itemcode(itemcode, app_id)
            return []
        ...
        print(f"[楽天APIエラー/例外] itemcode={itemcode} → {e}")
        return []
...
def get_rakuten_info(asins: dict) -> dict:
    ...
    for idx, (asin, info) in enumerate(asins.items(), start=1):
        ...
        if info is None:
            ...
            continue
        if info is None:
            print(f"[楽天SKIP] ASIN={asin} → infoがNone")
            continue
        ...
        if not items:
            ...
            if codes:
                keyword = " ".join(codes)
                print(f"[楽天Product候補結合キーワード検索] keyword: {keyword}")
                items = perform_rakuten_api_search(keyword, app_id)
        ...
        if not items:
            if brand and model:
                ...
                if len(keyword_brand_model) >= 4:
                    try:
                        print(f"[楽天再検索] メーカー＋型番で検索: {keyword_brand_model}")
                        items = perform_rakuten_api_search(keyword_brand_model, app_id)
                        time.sleep(sleep_time)
                    except Exception as ve:
                        print(f"[楽天SKIP] ASIN={asin} → メーカー＋型番検索時エラー: {ve}")
                        items = []
        ...
        if not items:
            ...
            try:
                ...
                print(f"[楽天最終検索] 短縮: {keyword_retry}")
                items = perform_rakuten_api_search(keyword_retry, app_id)
                time.sleep(sleep_time)
            except ValueError as ve:
                print(f"[楽天SKIP] ASIN={asin} → fallback不正(2): {ve}")
                items = []

        # 結果処理（省略せずしっかり残す）
        if not items:
            print(f"[楽天NO_HIT] ASIN={asin}, fallback={fallback}")
...
def search_rakuten_product_api(code):
    ...
    try:
        ...
        print(f'[楽天Product/Searchエラー] {code} → {ve}')
    finally:
        time.sleep(sleep_time)
    return []
...
def search_ichiba_from_product(jan=None, code=None, product_id=None):
    ...
    try:
        ...
        print(f'[楽天IchibaItem/Searchエラー] {code} {jan} {product_id} → {ve}')
    finally:
        time.sleep(sleep_time)
    return []
    ...
    #         print(f"[楽天多段検索] keyword={keyword}")
    #         try:
    #             items = perform_rakuten_api_search(keyword, app_id)
    #         except Exception as ve:
    #             print(f"[楽天SKIP] ASIN={asin} → keyword={keyword} エラー: {ve}")
    #             items = []
    #         time.sleep(sleep_time)
    #         if items:
    #             hit_flag = True
